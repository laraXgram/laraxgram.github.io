...
Eloquent relationships are defined as methods on your Eloquent model classes. Since relationships also serve as powerful [query builders](/v3/queries), defining relationships as methods provides powerful method chaining and querying capabilities. For example, we may chain additional query constraints on this `posts` relationship:
...
You may also provide a [collection](/v3/eloquent-collections) instance to the `whereBelongsTo` method. When doing so, LaraGram will retrieve models that belong to any of the parent models within the collection:
...
You are able to use any of the LaraGram [query builder's](/v3/queries) methods on the relationship, so be sure to explore the query builder documentation to learn about all of the methods that are available to you.
...
In most situations, you should use [logical groups](/v3/queries#logical-grouping) to group the conditional checks between parentheses:
...
You may also provide a [collection](/v3/eloquent-collections) instance to the `whereAttachedTo` method. When doing so, LaraGram will retrieve models that are attached to any of the models within the collection:
...
In this example, Eloquent will only eager load posts where the post's `title` column contains the word `code`. You may call other [query builder](/v3/queries) methods to further customize the eager loading operation:
...
You may also use the `findOrNew`, `firstOrNew`, `firstOrCreate`, and `updateOrCreate` methods to [create and update models on relationships](/v3/eloquent#upserts).

> [!NOTE]
> Before using the `create` method, be sure to review the [mass assignment](/v3/eloquent#mass-assignment) documentation.
