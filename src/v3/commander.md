...
Let's take a look at an example command. Note that we are able to request any dependencies we need via the command's `handle` method. The LaraGram [service container](/v3/container) will automatically inject all dependencies that are type-hinted in this method's signature:
...
In addition to receiving your command's arguments and options, command closures may also type-hint additional dependencies that you would like resolved out of the [service container](/v3/container):
...
> [!NOTE]
The comprehensive [LaraGram Prompts](/v3/prompts) documentation includes additional information on the available prompts and their usage.
...
> [!NOTE]
> [LaraGram Prompts](/v3/prompts) is a built-in package in the LaraGram console, written similar to [Laravel Prompts](https://laravel.com/docs/12.x/prompts). for adding beautiful and user-friendly forms to your command-line applications, with browser-like features including placeholder text and validation.
...
When Commander boots, all the commands in your application will be resolved by the [service container](/v3/container) and registered with Commander.
...
Using the `queue` method on the `Commander` facade, you may even queue Commander commands so they are processed in the background by your [queue workers](/v3/queues). Before using this method, make sure you have configured your queue and are running a queue listener:
