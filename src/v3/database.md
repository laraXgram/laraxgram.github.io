...
Almost every modern bot application interacts with a database. LaraGram makes interacting with databases extremely simple across a variety of supported databases using raw SQL, a [fluent query builder](/v3/queries), and the [Eloquent ORM](/v3/eloquent). Currently, LaraGram provides first-party support for five databases:
...
> [!NOTE]
> If you use the [LaraGram installer](/v3/installation#creating-a-laragram-project) to create your LaraGram application and select SQLite as your database, LaraGram will automatically create a `database/database.sqlite` file and run the default [database migrations](/v3/migrations) for you.
...
A common performance bottleneck of modern bot applications is the amount of time they spend querying databases. Thankfully, LaraGram can invoke a closure or callback of your choice when it spends too much time querying the database during a single request. To get started, provide a query time threshold (in milliseconds) and closure to the `whenQueryingForLongerThan` method. You may invoke this method in the `boot` method of a [service provider](/v3/providers):
...
> [!NOTE]
> The `DB` facade's transaction methods control the transactions for both the [query builder](/v3/queries) and [Eloquent ORM](/v3/eloquent).
...
To get started, you should schedule the `db:monitor` command to [run every minute](/v3/scheduling). The command accepts the names of the database connection configurations that you wish to monitor as well as the maximum number of open connections that should be tolerated before dispatching an event:
