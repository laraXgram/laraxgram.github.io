...
In order to use the `database` queue driver, you will need a database table to hold the jobs. Typically, this is included in LaraGram's default `0001_01_01_000002_create_jobs_table.php` [database migration](/v3/migrations); however, if your application does not contain this migration, you may use the `make:queue-table` Commander command to create it:
...
> [!NOTE]
> Job stubs may be customized using [stub publishing](/v3/commander#stub-customization).
...
In this example, note that we were able to pass an [Eloquent model](/v3/eloquent) directly into the queued job's constructor. Because of the `Queueable` trait that the job is using, Eloquent models and their loaded relationships will be gracefully serialized and unserialized when the job is processing.
...
The `handle` method is invoked when the job is processed by the queue. Note that we are able to type-hint dependencies on the `handle` method of the job. The LaraGram [service container](/v3/container) automatically injects these dependencies.

If you would like to take total control over how the container injects dependencies into the `handle` method, you may use the container's `bindMethod` method. The `bindMethod` method accepts a callback which receives the job and the container. Within the callback, you are free to invoke the `handle` method however you wish. Typically, you should call this method from the `boot` method of your `App\Providers\AppServiceProvider` [service provider](/v3/providers):
...
> [!WARNING]
> Unique jobs require a cache driver that supports [locks](/v3/cache#atomic-locks). Currently, the `memcached`, `redis`, `database`, `file`, and `array` cache drivers support atomic locks. In addition, unique job constraints do not apply to jobs within batches.
...
> [!NOTE]
> If you only need to limit the concurrent processing of a job, use the [WithoutOverlapping](/v3/queues#preventing-job-overlaps) job middleware instead.

<a name="encrypted-jobs"></a>
### Encrypted Jobs

LaraGram allows you to ensure the privacy and integrity of a job's data via [encryption](/v3/encryption). To get started, simply add the `ShouldBeEncrypted` interface to the job class. Once this interface has been added to the class, LaraGram will automatically encrypt your job before pushing it onto a queue:
...
As you can see, like [listen middleware](/v3/middleware), job middleware receive the job being processed and a callback that should be invoked to continue processing the job.
...
> [!NOTE]
> Job middleware can also be assigned to [queueable event listeners](/v3/events#queued-event-listeners).

<a name="rate-limiting"></a>
### Rate Limiting

Although we just demonstrated how to write your own rate limiting job middleware, LaraGram actually includes a rate limiting middleware that you may utilize to rate limit jobs. Like [listen rate limiters](/v3/listening#defining-rate-limiters), job rate limiters are defined using the `RateLimiter` facade's `for` method.
...
> [!WARNING]
> The `WithoutOverlapping` middleware requires a cache driver that supports [locks](/v3/cache#atomic-locks). Currently, the `memcached`, `redis`, `database`, `file`, and `array` cache drivers support atomic locks.
...
Without pruning, the `job_batches` table can accumulate records very quickly. To mitigate this, you should [schedule](/v3/scheduling) the `queue:prune-batches` Commander command to run daily:
...
> [!NOTE]
> The queue uses the [cache](/v3/cache) to store restart signals, so you should verify that a cache driver is properly configured for your application before using this feature.
...
To get started, you should schedule the `queue:monitor` command to [run every minute](/v3/scheduling). The command accepts the names of the queues you wish to monitor as well as your desired job count threshold:
...
Using the `before` and `after` methods on the `Queue` [facade](/v3/facades), you may specify callbacks to be executed before or after a queued job is processed. These callbacks are a great opportunity to perform additional logging or increment statistics for a dashboard. Typically, you should call these methods from the `boot` method of a [service provider](/v3/providers). For example, we may use the `AppServiceProvider` that is included with LaraGram:
...
Using the `looping` method on the `Queue` [facade](/v3/facades), you may specify callbacks that execute before the worker attempts to fetch a job from a queue. For example, you might register a closure to rollback any transactions that were left open by a previously failed job:
