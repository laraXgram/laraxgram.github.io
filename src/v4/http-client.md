...
The `get` method returns an instance of `LaraGram\Http\Client\Response`, which provides a variety of methods that may be used to inspect the response:

```php
$response->body() : string;
$response->json($key = null, $default = null) : mixed;
$response->object() : object;
$response->collect($key = null) : LaraGram\Support\Collection;
...
The `LaraGram\Http\Client\Response` object also implements the PHP `ArrayAccess` interface, allowing you to access JSON response data directly on the response:
...
If the given timeout is exceeded, an instance of `LaraGram\Http\Client\ConnectionException` will be thrown.
...
If you would like the HTTP client to automatically retry the request if a client or server error occurs, you may use the `retry` method. The `retry` method accepts the maximum number of times the request should be attempted and the number of milliseconds that LaraGram should wait in between attempts:
...
> [!WARNING]
> If all of the requests fail because of a connection issue, a `LaraGram\Http\Client\ConnectionException` will still be thrown even when the `throw` argument is set to `false`.
...
If you have a response instance and would like to throw an instance of `LaraGram\Http\Client\RequestException` if the response status code indicates a client or server error, you may use the `throw` or `throwIf` methods:
...
The `LaraGram\Http\Client\RequestException` instance has a public `$response` property which will allow you to inspect the returned response.
...
Thankfully, you may accomplish this using the `pool` method. The `pool` method accepts a closure which receives an `LaraGram\Http\Client\Pool` instance, allowing you to easily add requests to the request pool for dispatching:
...
Another way of working with concurrent requests in LaraGram is to use the `batch` method. Like the `pool` method, it accepts a closure which receives an `LaraGram\Http\Client\Batch` instance, allowing you to easily add requests to the request pool for dispatching, but it also allows you to define completion callbacks:

```php
use LaraGram\Http\Client\Batch;
...
After a `batch` is started by calling the `send` method, you can't add new requests to it. Trying to do so will result in a `LaraGram\Http\Client\BatchInProgressException` exception being thrown.
...
The `LaraGram\Http\Client\Batch` instance that is provided to batch completion callbacks has a variety of properties and methods to assist you in interacting with and inspecting a given batch of requests:
...
When the `defer` method is invoked, the batch of requests is not executed immediately. Instead, LaraGram will execute the batch after the current application request's HTTP response has been sent to the user, keeping your application feeling fast and responsive:
...
Many LaraGram services provide functionality to help you easily and expressively write tests, and LaraGram's HTTP client is no exception. The `Http` facade's `fake` method allows you to instruct the HTTP client to return stubbed / dummy responses when requests are made.
...
Sometimes you may need to test your application's behavior if the HTTP client encounters an `LaraGram\Http\Client\ConnectionException` when attempting to make a request. You can instruct the HTTP client to throw a connection exception using the `failedConnection` method:
...
To test your application's behavior if a `LaraGram\Http\Client\RequestException` is thrown, you may use the `failedRequest` method:
...
If you require more complicated logic to determine what responses to return for certain endpoints, you may pass a closure to the `fake` method. This closure will receive an instance of `LaraGram\Http\Client\Request` and should return a response instance. Within your closure, you may perform whatever logic is necessary to determine what type of response to return:
...
The `assertSent` method accepts a closure which will receive an `LaraGram\Http\Client\Request` instance and should return a boolean value indicating if the request matches your expectations. In order for the test to pass, at least one request must have been issued matching the given expectations:
...
You may use the `recorded` method to gather all requests and their corresponding responses. The `recorded` method returns a collection of arrays that contains instances of `LaraGram\Http\Client\Request` and `LaraGram\Http\Client\Response`:
...
Additionally, the `recorded` method accepts a closure which will receive an instance of `LaraGram\Http\Client\Request` and `LaraGram\Http\Client\Response` and may be used to filter request / response pairs based on your expectations:
