...
LaraGram's `LaraGram\Http\Request` class provides an object-oriented way to interact with the current HTTP request being handled by your application as well as retrieve the input, cookies, and files that were submitted with the request.
...
To obtain an instance of the current HTTP request via dependency injection, you should type-hint the `LaraGram\Http\Request` class on your route closure or controller method. The incoming request instance will automatically be injected by the LaraGram [service container](/master/container):
...
As mentioned, you may also type-hint the `LaraGram\Http\Request` class on a route closure. The service container will automatically inject the incoming request into the closure when it is executed:
...
You may still type-hint the `LaraGram\Http\Request` and access your `id` route parameter by defining your controller method as follows:
...
The `LaraGram\Http\Request` instance provides a variety of methods for examining the incoming HTTP request and extends the `LaraGram\Http\BaseRequest` class. We will discuss a few of the most important methods below.
...
Using the `routeIs` method, you may determine if the incoming request has matched a [named route](/master/routing#named-routes):
...
$request->httpHost(); // localhost:9000
$request->schemeAndHttpHost(); // http://localhost:9000
```
...
You may retrieve a request header from the `LaraGram\Http\Request` instance using the `header` method. If the header is not present on the request, `null` will be returned. However, the `header` method accepts an optional second argument that will be returned if the header is not present on the request:
...
LaraGram provides several methods for inspecting the incoming request's requested content types via the `Accept` header. First, the `getAcceptableContentTypes` method will return an array containing all of the content types accepted by the request:
...
Using the `collect` method, you may retrieve all of the incoming request's input data as a [collection](/master/collections):
...
Using a few simple methods, you may access all of the user input from your `LaraGram\Http\Request` instance without worrying about which HTTP verb was used for the request. Regardless of the HTTP verb, the `input` method may be used to retrieve user input:
...
Instead of retrieving the request's input data as a primitive `string`, you may use the `string` method to retrieve the request data as an instance of [LaraGram\Support\Stringable](/master/strings):
...
You may also access user input using dynamic properties on the `LaraGram\Http\Request` instance. For example, if one of your application's forms contains a `name` field, you may access the value of the field like so:

```php
$name = $request->name;
```

When using dynamic properties, LaraGram will first look for the parameter's value in the request payload. If it is not present, LaraGram will search for the field in the matched route's parameters.
...
LaraGram allows you to keep input from one request during the next request. This feature is particularly useful for re-populating forms after detecting validation errors. However, if you are using LaraGram's included [validation features](/master/validation), it is possible that you will not need to manually use these session input flashing methods directly, as some of LaraGram's built-in validation facilities will call them automatically.

<a name="flashing-input-to-the-session"></a>
#### Flashing Input to the Session

The `flash` method on the `LaraGram\Http\Request` class will flash the current input to the [session](/master/session) so that it is available during the user's next request to the application:
...
To retrieve flashed input from the previous request, invoke the `old` method on an instance of `LaraGram\Http\Request`. The `old` method will pull the previously flashed input data from the [session](/master/session):

```php
$username = $request->old('username');
```

LaraGram also provides a global `old` helper. If you are displaying old input within a [Blade template](/master/blade), it is more convenient to use the `old` helper to repopulate the form. If no old input exists for the given field, `null` will be returned:
...
All cookies created by the LaraGram framework are encrypted and signed with an authentication code, meaning they will be considered invalid if they have been changed by the client. To retrieve a cookie value from the request, use the `cookie` method on an `LaraGram\Http\Request` instance:
...
You may retrieve uploaded files from an `LaraGram\Http\Request` instance using the `file` method or using dynamic properties. The `file` method returns an instance of the `LaraGram\Http\UploadedFile` class, which extends the PHP `SplFileInfo` class and provides a variety of methods for interacting with the file:
...
To store an uploaded file, you will typically use one of your configured [filesystems](/master/filesystem). The `UploadedFile` class has a `store` method that will move an uploaded file to one of your disks, which may be a location on your local filesystem or a cloud storage location like Amazon S3.
...
> [!NOTE]
> For more information about file storage in LaraGram, check out the complete [file storage documentation](/master/filesystem).
...
To solve this, you may enable the `LaraGram\Http\Middleware\TrustProxies` middleware that is included in your LaraGram application, which allows you to quickly customize the load balancers or proxies that should be trusted by your application. Your trusted proxies should be specified using the `trustProxies` middleware method in your application's `bootstrap/app.php` file:
...
By default, LaraGram will respond to all requests it receives regardless of the content of the HTTP request's `Host` header. In addition, the `Host` header's value will be used when generating absolute URLs to your application during a web request.

Typically, you should configure your web server, such as Nginx or Apache, to only send requests to your application that match a given hostname. However, if you do not have the ability to customize your web server directly and need to instruct LaraGram to only respond to certain hostnames, you may do so by enabling the `LaraGram\Http\Middleware\TrustHosts` middleware for your application.
