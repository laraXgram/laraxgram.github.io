...
In LaraGram, exception reporting is used to log exceptions. By default, exceptions will be logged based on your [logging](/master/logging) configuration. However, you are free to log exceptions however you wish.
...
> [!NOTE]
> To customize the exception reporting for a given exception, you may also utilize [reportable exceptions](/master/errors#renderable-exceptions).
...
When messages are written to your application's [logs](/master/logging), the messages are written at a specified [log level](/master/logging#log-levels), which indicates the severity or importance of the message being logged.
...
By default, the LaraGram exception handler will convert exceptions into an HTTP response for you. However, you are free to register a custom rendering closure for exceptions of a given type. You may accomplish this by using the `render` exception method in your application's `bootstrap/app.php` file.

The closure passed to the `render` method should return an instance of `LaraGram\Http\Response`, which may be generated via the `response` helper. LaraGram will determine what type of exception the closure renders by examining the type-hint of the closure:
...
You may also use the `render` method to override the rendering behavior for built-in LaraGram or Symfony exceptions such as `NotFoundHttpException`. If the closure given to the `render` method does not return a value, LaraGram's default exception rendering will be utilized:
...
When rendering an exception, LaraGram will automatically determine if the exception should be rendered as an HTML or JSON response based on the `Accept` header of the request. If you would like to customize how LaraGram determines whether to render HTML or JSON exception responses, you may utilize the `shouldRenderJsonWhen` method:
...
Rarely, you may need to customize the entire HTTP response rendered by LaraGram's exception handler. To accomplish this, you may register a response customization closure using the `respond` method:
...
If your exception extends an exception that is already renderable, such as a built-in LaraGram or Symfony exception, you may return `false` from the exception's `render` method to render the exception's default HTTP response:
...
> [!NOTE]
> You may type-hint any required dependencies of the `report` method and they will automatically be injected into the method by LaraGram's [service container](/master/container).
...
LaraGram makes it easy to display custom error pages for various HTTP status codes. For example, to customize the error page for 404 HTTP status codes, create a `resources/views/errors/404.blade.php` view template. This view will be rendered for all 404 errors generated by your application. The views within this directory should be named to match the HTTP status code they correspond to. The `Symfony\Component\HttpKernel\Exception\HttpException` instance raised by the `abort` function will be passed to the view as an `$exception` variable:

```blade
<h2>{{ $exception->getMessage() }}</h2>
```

You may publish LaraGram's default error page templates using the `vendor:publish` Commander command. Once the templates have been published, you may customize them to your liking:

```shell
php laragram vendor:publish --tag=laravel-errors
```
...
When defining fallback error pages, the fallback pages will not affect `404`, `500`, and `503` error responses since LaraGram has internal, dedicated pages for these status codes. To customize the pages rendered for these status codes, you should define a custom error page for each of them individually.
