...
LaraGram provides several helpers to assist you in generating URLs for your application. These helpers are primarily helpful when building links in your templates and API responses, or when generating redirect responses to another part of your application.
...
```php
echo url()->query('/posts', ['search' => 'LaraGram']);

// https://example.com/posts?search=LaraGram

echo url()->query('/posts?sort=latest', ['search' => 'LaraGram']);

// http://example.com/posts?sort=latest&search=LaraGram
```
...
If no path is provided to the `url` helper, an `LaraGram\Routing\UrlGenerator` instance is returned, allowing you to access information about the current URL:
...
LaraGram allows you to easily create "signed" URLs to named routes. These URLs have a "signature" hash appended to the query string which allows LaraGram to verify that the URL has not been modified since it was created. Signed URLs are especially useful for routes that are publicly accessible yet need a layer of protection against URL manipulation.
...
If you would like to generate a temporary signed route URL that expires after a specified amount of time, you may use the `temporarySignedRoute` method. When LaraGram validates a temporary signed route URL, it will ensure that the expiration timestamp that is encoded into the signed URL has not elapsed:
...
To verify that an incoming request has a valid signature, you should call the `hasValidSignature` method on the incoming `LaraGram\Http\Request` instance:
...
LaraGram's `Uri` class provides a convenient and fluent interface for creating and manipulating URIs via objects. This class wraps the functionality provided by the underlying League URI package and integrates seamlessly with LaraGram's routing system.
...
->withMiddleware(function (Middleware $middleware): void {
    $middleware->prependToPriorityList(
        before: \LaraGram\Routing\Middleware\SubstituteBindings::class,
