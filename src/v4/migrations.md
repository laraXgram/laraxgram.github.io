...
The LaraGram `Schema` [facade](/v4/facades) provides database agnostic support for creating and manipulating tables across all of LaraGram's supported database systems. Typically, migrations will use this facade to create and modify database tables and columns.

<a name="generating-migrations"></a>
## Generating Migrations

You may use the `make:migration` [Commander command](/v4/commander) to generate a database migration. The new migration will be placed in your `database/migrations` directory. Each migration filename contains a timestamp that allows LaraGram to determine the order of the migrations:
...
> [!NOTE]
> Migration stubs may be customized using [stub publishing](/v4/commander#stub-customization).
...
This method is intended to be used when defining the columns necessary for a polymorphic [Eloquent relationship](/v4/eloquent-relationships). In the following example, `taggable_id` and `taggable_type` columns would be created:
...
The `rememberToken` method creates a nullable, `VARCHAR(100)` equivalent column that is intended to store the current "remember me" [authentication token](/v4/authentication#remembering-users):
...
This method is intended to be used when defining the columns necessary for a polymorphic [Eloquent relationship](/v4/eloquent-relationships) that use ULID identifiers. In the following example, `taggable_id` and `taggable_type` columns would be created:
...
This method is intended to be used when defining the columns necessary for a polymorphic [Eloquent relationship](/v4/eloquent-relationships) that use UUID identifiers. In the following example, `taggable_id` and `taggable_type` columns would be created:
...
> [!WARNING]
> SQLite disables foreign key constraints by default. When using SQLite, make sure to [enable foreign key support](/v4/database#configuration) in your database configuration before attempting to create them in your migrations.

<a name="events"></a>
## Events

For convenience, each migration operation will dispatch an [event](/v4/events). All of the following events extend the base `LaraGram\Database\Events\MigrationEvent` class:
