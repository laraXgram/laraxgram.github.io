...
When testing your application or seeding your database, you may need to insert a few records into your database. Instead of manually specifying the value of each column, LaraGram allows you to define a set of default attributes for each of your [Eloquent models](/src/eloquent.mdt.md) using model factories.
...
To create a factory, execute the `make:factory` [Commander command](/src/commander.md):
...
If your Eloquent model can be [soft deleted](/src/eloquent.mdt.md#soft-deleting), you may invoke the built-in `trashed` state method to indicate that the created model should already be "soft deleted". You do not need to manually define the `trashed` state as it is automatically available to all factories:
...
> [!NOTE]
> [Mass assignment protection](/src/eloquent.mdt.md#mass-assignment) is automatically disabled when creating models using factories.
...
[Polymorphic relationships](/src/eloquent-relationships.mds.md#polymorphic-relationships) may also be created using factories. Polymorphic "morph many" relationships are created in the same way as typical "has many" relationships. For example, if an `App\Models\Post` model has a `morphMany` relationship with an `App\Models\Comment` model:
