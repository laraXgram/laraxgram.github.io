...
Thankfully, many of the classes you will be writing when building a LaraGram application automatically receive their dependencies via the container, including [controllers](/src/controllers.mds.md), [event listeners](/src/events.mds.md), [middleware](/src/middleware.mde.md), and more. Additionally, you may type-hint dependencies in the `handle` method of [queued jobs](/src/queues.mds.md). Once you taste the power of automatic and zero configuration dependency injection it feels impossible to develop without it.
...
In many cases, thanks to automatic dependency injection and [facades](/src/facades.mds.md), you can build LaraGram applications without **ever** manually binding or resolving anything from the container. **So, when would you ever manually interact with the container?** Let's examine two situations.

First, if you write a class that implements an interface and you wish to type-hint that interface on a listen or class constructor, you must [tell the container how to resolve that interface](#binding-interfaces-to-implementations). Secondly, if you are [writing a LaraGram package](/src/packages.mds.md) that you plan to share with other LaraGram developers, you may need to bind your package's services into the container.
...
Almost all of your service container bindings will be registered within [service providers](/src/providers.mds.md), so most of these examples will demonstrate using the container in that context.
...
As mentioned, you will typically be interacting with the container within service providers; however, if you would like to interact with the container outside of a service provider, you may do so via the `App` [facade](/src/facades.mds.md):
...
Sometimes you may have two classes that utilize the same interface, but you wish to inject different implementations into each class. For example, two controllers may depend on different implementations of the `LaraGram\Contracts\Filesystem\Filesystem` [contract](/src/contracts.mds.md). LaraGram provides a simple, fluent interface for defining this behavior:
...
For example, the `Storage` attribute may be used to inject a specific [storage disk](/src/filesystem.mdm.md):
...
If you are outside of a service provider in a location of your code that does not have access to the `$app` variable, you may use the `App` [facade](/src/facades.mds.md) or the `app` [helper](/src/helpers.mds.md#method-app) to resolve a class instance from the container:
...
Alternatively, and importantly, you may type-hint the dependency in the constructor of a class that is resolved by the container, including [controllers](/src/controllers.mds.md), [event listeners](/src/events.mds.md), [middleware](/src/middleware.mde.md), and more. Additionally, you may type-hint dependencies in the `handle` method of [queued jobs](/src/queues.mds.md). In practice, this is how most of your objects should be resolved by the container.
