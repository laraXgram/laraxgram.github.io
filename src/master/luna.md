...
Luna is the official adapter that lets you build modern, single-page style frontends for your LaraGram application using React, Vue, or Svelte — without building an API, without client-side routing configuration, and without losing any of the server-side conveniences you already know: [routing](/master/routing), [controllers](/master/controllers), [middleware](/master/middleware), [validation](/master/validation), and [authorization](/master/authorization).

Luna has two halves that ship together: the LaraGram adapter you see in this page, and the Luna-JS client runtime that drives the browser. You write your routes and controllers exactly as you always have. Instead of returning a Blade [view](/master/views) or JSON, a controller returns a Luna *response*, naming a JavaScript page component and the props it should receive:
...
Luna sits between LaraGram's [router](/master/routing) and your JavaScript framework. The lifecycle of a request looks like this:
...
| `version` | The current [asset version](/master/luna-frontend#asset-versioning), used to force a full reload when assets change. |
...
The quickest way — and the recommended one — is to create the application with the [LaraGram installer](/master/installation#installing-php) and pick one of the [starter kits](/master/starter-kits). Each kit ships with Luna already wired up: the PHP adapter, the npm packages, the root template, Vite, and an example page:
...
Luna needs PHP 8.2 or newer with the `json` and `sodium` extensions, and a LaraGram 4 application with the [web layer](/master/routing) enabled.

Publish the configuration file. This creates `config/luna.php`, where you configure [server-side rendering](/master/luna-frontend#server-side-rendering), page paths, history encryption, and [Telegram](/master/luna-tma#configuration) options:
...
Luna needs a single root [Blade](/master/blade) template that loads your compiled assets and provides the mounting point. By convention it lives at `resources/views/app.blade.php`. Use the `@luna` directive for the root element and `@lunaHead` for server-rendered `<head>` content:
...
- `@lunaHead` renders any `<title>`, `<meta>`, and other head elements produced by [server-side rendering](/master/luna-frontend#server-side-rendering).
- `@vite` loads your compiled JS/CSS via the [Vite](/master/vite) plugin.

</div>

If you name your root template something other than `app`, tell Luna about it globally in a [service provider](/master/providers):
...
Install the [Vite](/master/vite) plugin as a dev dependency — it wires up page auto-resolution and the SSR dev server:
...
The equivalent Vue and Svelte entry points — plus the Vite plugin's `pages` shorthand that removes the `import.meta.glob` boilerplate — are covered in [Frontend Setup](/master/luna-frontend). Configure the Vite plugin in `vite.config.js`:
...
Wire a [route](/master/routing) to it. Any controller returning `Luna::render()` works, but for prop-less pages Luna adds a `Route::luna()` shortcut that renders a component directly:
...
That's a complete round trip: a route, a controller returning a named component with props, and a page component that renders them. From here, add [links and visits](/master/luna-routing), [forms](/master/luna-forms), and richer [props](/master/luna-pages).
...
- **[Pages & Props](/master/luna-pages)** — rendering components, shared props, deferred/optional/merged props, and partial reloads.
- **[Routing & Visits](/master/luna-routing)** — the `<Link>` component, manual router visits, redirects, prefetching, polling, and history.
- **[Forms](/master/luna-forms)** — the `useForm` helper, the `<Form>` component, validation errors, file uploads, and [Precognition](/master/precognition).
- **[Frontend Setup](/master/luna-frontend)** — per-framework setup, persistent layouts, `<Head>`, title/meta, code splitting, the [Vite](/master/vite) plugin, and server-side rendering.
- **[Telegram Mini Apps](/master/luna-tma)** — building TMAs: init-data validation, the `telegram` guard, the reactive store, and native-button integration.
- **[Telegram Features](/master/luna-tma-features)** — CloudStorage, biometrics, location, sensors, haptics, popups, sharing, invoices, and the test mock.
