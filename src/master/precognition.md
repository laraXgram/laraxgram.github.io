...
> [!NOTE]
> As of Luna, Precognition support is built-in. Please consult the [Luna Forms documentation](/master/luna-forms) for more information. Earlier Luna versions require Precognition 0.x.
...
use LaraGram\Foundation\Http\Middleware\HandlePrecognitiveRequests;
...
Next, you should install the LaraGram Precognition frontend helpers for Vue via NPM:

```shell
npm install laragram-precognition-vue
```

With the LaraGram Precognition package installed, you can now create a form object using Precognition's `useForm` function, providing the HTTP method (`post`), the target URL (`/users`), and the initial form data.

Then, to enable live validation, invoke the form's `validate` method on each input's `change` event, providing the input's name:

```vue
<script setup>
import { useForm } from 'laragram-precognition-vue';
...
Now, as the form is filled by the user, Precognition will provide live validation output powered by the validation rules in the route's form request. When the form's inputs are changed, a debounced "precognitive" validation request will be sent to your LaraGram application. You may configure the debounce timeout by calling the form's `setValidationTimeout` function:
...
Next, you should install the LaraGram Precognition frontend helpers for React via NPM:

```shell
npm install laragram-precognition-react
```
...
```jsx
import { useForm } from 'laragram-precognition-react';
...
Next, you should install the LaraGram Precognition frontend helpers for Alpine via NPM:

```shell
npm install laragram-precognition-alpine
```
...
import Precognition from 'laragram-precognition-alpine';
...
With the LaraGram Precognition package installed and registered, you can now create a form object using Precognition's `$form` "magic", providing the HTTP method (`post`), the target URL (`/users`), and the initial form data.
...
The Precognition validation libraries use the [Axios](https://github.com/axios/axios) HTTP client to send requests to your application's backend. For convenience, the Axios instance may be customized if required by your application. For example, when using the `laragram-precognition-vue` library, you may add additional request headers to each outgoing request in your application's `resources/js/app.js` file:

```js
import { client } from 'laragram-precognition-vue';
...
By default, LaraGram Precognition does not upload or validate files during a precognitive validation request. This ensure that large files are not unnecessarily uploaded multiple times.
...
If you would like to make precognitive requests in your tests, LaraGram's `TestCase` includes a `withPrecognition` helper which will add the `Precognition` request header.
