...
To generate a seeder, execute the `make:seeder` [Commander command](/commander). All seeders generated by the framework will be placed in the `database/seeders` directory:

```shell
php laragram make:seeder UserSeeder
```

A seeder class only contains one method by default: `run`. This method is called when the `db:seed` [Commander command](/commander) is executed. Within the `run` method, you may insert data into your database however you wish. You may use the [query builder](/queries.md) to manually insert data or you may use [Eloquent model factories](/eloquent-factories.md).
